...
<html><head>
    ...
    <title>Laporan Keuangan | Buku Kas</title>
    </head>
    <body>
        ...
        <tr>
            <td align="center">
                <span style="line-height:1.6; font-weight: bold;">APLIKASI PENGOLAAN UANGKAS Universitas Dipanegara
                    ...
                    <br><p>JL.PERINTIS KEMERDEKAAN NO.!@#!@$</p> 
                </span>
            </td>
        </tr>
        </table>
        ...
        <thead class="thead-dark">
            <tr>
                ...
                foreach ($uangkas as $uk):
                    $total = $uk->pemasukan - $uk->pengeluaran;
                    $subtotal += $total?>
                <tbody>
                <tr>
                <td><?php echo $id++ ?></td>
                <td><?php echo $uk->nama_input ?></td>
                <td>Rp.<?php echo number_format($uk->pemasukan,0,',','.')?></td>
                <td><?php echo $uk->tanggal_masuk ?></td>
                <td>Rp.<?php echo number_format($uk->pengeluaran,0,',','.')?></td>
                <td><?php echo $uk->tanggal_keluar ?></td>
                <td><?php echo $uk->deskripsi_pemasukan ?></td>
                <td><?php echo $uk->deskripsi_pengeluaran ?></td>
                <td>Rp.<?php echo number_format($total,0,',','.')?></td>
                </tr>
                </tbody>
                <?php endforeach; ?>
                <td class=""><strong >TOTAL : </strong>Rp.<?php echo number_format($subtotal,0,',','.')?></td>
        </table>
